...
Usage:
    ...
    python demo_webcam.py --model yolov8s-pose --classes person
    ...
    def process_frame(self, frame):
        ...
        if self.tracking_enabled:
            ...
            if target and target.disappeared == 0:
                # [新增] 绘制 Pose 骨架
                # Tracker保存的TrackedObject，我们需要从中获取最新的关键点
                # 注意: src/tracker.py 中的 TrackedObject 会保存最近一次 update 的 detection 的数据
                # 我们需要确保 Tracker 也保存 keypoints 属性
                # 为了简化，这里我们假设 target 对象上我们动态绑定了 keypoints，
                # 或者因为 TrackedObject 是从 Detection 初始化的，我们可以让它携带。
                # 最好去修改 tracker.py, 但为了兼容性, 我们可以直接从 detection (如果它就是target) 获取
                # 或者在 detector 返回的 detection 对象里直接画
                
                # 由于 TrackedObject 默认没有 keypoints 字段 (除非修改 tracker.py), 
                # 但 Python 对象是动态的，我们可以在 detection 更新到 tracker 时，附带上 keypoints
                # 这里为了简单，如果 detection 存在且就是当前 target，直接用 detection 的 keypoints
                current_keypoints = getattr(detection, 'keypoints', None) if detection else None
                
                # 如果使用了 Pose 模型，绘制骨架
                if current_keypoints is not None:
                     display_frame = draw_pose(display_frame, current_keypoints, color=(255, 0, 255))
                     # 如果是 Pose 模式，边框颜色改一下以示区别
                     color = (255, 0, 255)
                else:
                     color = (0, 255, 0)
