...
class ObjectTracker:
    ...
    Can also work in 'pass-through' mode if YOLO handles the tracking.
    """
    ...
    def update(self, detections: List[Detection]) -> Dict[int, TrackedObject]:
        ...
        """
        # [修改] 检查是否使用 YOLO 内部的 tracking 结果
        if self.config.tracking_method == "yolo":
            return self._update_from_yolo(detections)
        else:
            return self._update_custom(detections)

    def _update_from_yolo(self, detections: List[Detection]) -> Dict[int, TrackedObject]:
        """Update logic when using YOLO's internal tracker."""
        current_frame_ids = set()

        for det in detections:
            # 只有当 Detection 包含 track_id 时才有效 (YOLO persist=True 模式)
            if det.track_id is not None:
                ...
                if track_id in self.objects:
                    # 更新现有对象
                    self.objects[track_id].update(det)
                else:
                    # 注册新对象，直接使用 YOLO 的 ID
                    self.objects[track_id] = TrackedObject(track_id, det)
        
        # 处理消失的对象
        # 在 YOLO 模式下，如果 YOLO 没返回某个 ID，我们就认为它消失了
    ...
    def _update_custom(self, detections: List[Detection]) -> Dict[int, TrackedObject]:
        """Original Centroid-based update logic."""
...
class SingleObjectTracker:
    ...
    def update(self, detection: Optional[Detection]) -> Optional[TrackedObject]:
        ...
        if self.target is None:
            # Check if detection carries a YOLO ID
            obj_id = detection.track_id if detection.track_id is not None else 0
            self.target = TrackedObject(obj_id, detection)
        else:
            # Update existing target
            # If tracking method is YOLO, we should ideally verify the ID matches
            # But for SingleObjectTracker (simplest case), we assume the detector
            # logic already filtered for 'closest to center', so we just update.
            self.target.update(detection)
            # Ensure ID stays synced if YOLO changes it (rare in single mode)
            if detection.track_id is not None:
                self.target.id = detection.track_id
