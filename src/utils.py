...
def draw_pose(
    ...
    Args:
        ...
        confidences: Array of shape (17,) with keypoint confidences
    """
    ...
    # Skeleton connections (start_idx, end_idx)
    skeleton = [
    ...
    # Threshold for displaying a keypoint
    conf_thresh = 0.5
    ...
    for p1_idx, p2_idx in skeleton:
        ...
        x1, y1 = map(int, keypoints[p1_idx])
        x2, y2 = map(int, keypoints[p2_idx])
        
        # Check confidence if available
        if confidences is not None:
            if confidences[p1_idx] < conf_thresh or confidences[p2_idx] < conf_thresh:
                continue
                
        # Don't draw if points are at (0,0) which usually means not detected
    ...
    for i, point in enumerate(keypoints):
        ...
        if confidences is not None and confidences[i] < conf_thresh:
            continue
