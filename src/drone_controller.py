...
class DroneController:
    ...
    def track_target(self, target: Optional[TrackedObject]) -> None:
        ...
        #target_area = frame_area * 0.6 #室外

#        target_area = frame_area * 0.3
        target_area = frame_area * 0.25
        ...
        #改2
        # 1. 左右 (LR): 目标在右(error_x>0) -> 应向右飞(正) -> 去掉负号
        cmd_lr = self.pid_x.update(error_x) 
        
        # 2. 前后 (FB): 目标太近(error_z>0) -> 应后退(负) -> 加上负号
        cmd_fb = -self.pid_z.update(error_z) 
        
        # 3. 上下 (UD): 目标在下(error_y>0) -> 飞机太高应下降(负) -> 保持负号 (这个是对的)
        cmd_ud = -self.pid_y.update(error_y) 
        
        # 4. 旋转 (Yaw): 目标在右(error_x>0) -> 应顺时针转(正) -> 去掉负号
        cmd_yaw = self.pid_x.update(error_x) * 0.5
