...
class Config:
    ...
    # [核心] 这些类别将作为 Prompt 输入给 YOLO-World
    target_classes: list = field(default_factory=lambda: ["person", "ball", "sports ball"])
    
    # [新增] 跟踪模式选择
    # "custom": 使用 src/tracker.py 中的距离匹配算法 (简单，完全可控)
    # "yolo": 使用 YOLOv8 内置的 BoT-SORT/ByteTrack (更稳健，抗遮挡能力强)
    tracking_method: str = "custom" 
...
class ConfigBuilder:
    ...
    def with_tracking_method(self, method: str) -> "ConfigBuilder":
        """Set tracking method ('custom' or 'yolo')."""
...
def get_webcam_config() -> Config:
    ...
    config = Config()
    # [修改] 默认使用 YOLO-Worldv2
    config.model_name = "yolov8s-worldv2"
    config.confidence_threshold = 0.4  # Lower threshold for demo
    config.tracking_method = "custom" # Default to custom, user can switch
